...
APP_VERSION = "v2.4.3-minimal-debug"
import requests
...
@app.route("/debug")
def debug_dashboard():
    """Simple debug dashboard - minimal version that won't crash."""
    try:
...
<html>
<head>
    <title>BTTS Debug</title>
    <style>
        body {{ font-family: monospace; padding: 20px; background: #1a1a1a; color: #00ff00; }}
        .section {{ margin: 20px 0; padding: 15px; background: #2a2a2a; border: 1px solid #00ff00; border-radius: 5px; }}
        h2 {{ color: #00ffff; }}
        .ok {{ color: #44ff44; }}
        .error {{ color: #ff4444; }}
        a {{ color: #00ff00; }}
...
<body>
    <h1>🔍 BTTS Debug Dashboard</h1>
    <p>Version: {APP_VERSION} | Time: {datetime.now().isoformat()}</p>
    <p><a href="/admin">← Back to Admin</a> | <a href="/debug/bbc/detailed?league=sco.4">BBC Debug</a></p>
...
"""
        ...
        # Test ESPN
        try:
            resp = requests.get("https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/scoreboard", timeout=5)
            events = resp.json().get("events", [])
            html += f'<p class="ok">✅ Status: {resp.status_code}</p>'
            html += f'<p>Events found: {len(events)}</p>'
        except Exception as e:
            html += f'<p class="error">❌ Error: {str(e)}</p>'
        
        html += '</div><div class="section"><h2>🏴󠁧󠁢󠁳󠁣󠁴󠁿 BBC Scraper Test</h2>'
        
        # Test BBC
        try:
            from bbc_scraper import scrape_bbc_fixtures
            fixtures = scrape_bbc_fixtures('sco.4')
            if fixtures:
                html += f'<p class="ok">✅ Fixtures found: {len(fixtures)}</p><ul>'
                for f in fixtures[:5]:
                    html += f"<li>{f.get('home_team')} v {f.get('away_team')}</li>"
                html += '</ul>'
            else:
                html += '<p class="error">⚠️ No fixtures found</p>'
        except Exception as e:
            html += f'<p class="error">❌ Error: {str(e)}</p>'
        
        html += '</div><div class="section"><h2>📝 Files</h2>'
        
        # Check files
        try:
            manual_file = os.path.join(os.path.dirname(__file__), "manual_matches.json")
            assign_file = os.path.join(os.path.dirname(__file__), "assignments.json")
            html += f'<p>Manual matches: {"✅ Exists" if os.path.exists(manual_file) else "❌ Not found"}</p>'
            html += f'<p>Assignments: {"✅ Exists" if os.path.exists(assign_file) else "❌ Not found"}</p>'
        except Exception as e:
            html += f'<p class="error">Error: {str(e)}</p>'
        
        html += '</div><div class="section"><h2>💻 System</h2>'
        
        # System info
        try:
            import sys
            html += f'<p>Python: {sys.version.split()[0]}</p>'
            html += f'<p>Platform: {sys.platform}</p>'
        except Exception as e:
            html += f'<p class="error">Error: {str(e)}</p>'
        ...
        import traceback
        return f"""<html><body style="font-family:monospace;padding:20px;background:#1a1a1a;color:#ff4444">
        <h1>❌ Debug Error</h1>
        <pre>{str(e)}\n\n{traceback.format_exc()}</pre>
        <a href="/admin" style="color:#00ff00">Back</a></body></html>"""
@app.route("/admin")
def admin_page():
